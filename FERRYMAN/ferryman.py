...
def read_sensor_data():
    ...
    print(
        f"[SENSOR TRANSMIT] Reading raw value from Asherah : UINT {pzr_press} || {scaled_press}MPa."
    )
...
def write_control_commands():
    ...
    holding_reg = plc_client.read_holding_registers(
        1026, 3
    )  # READ 3 CONSECUTIVE HOLDING REGISTERS IN THE PLC
    pzr_main_heater = holding_reg.registers[0]
    scaled_main_heater = pzr_main_heater / 65535 * 100
    print(
        f"[CONTROL COMMAND] Reading control from PLC : UINT {pzr_main_heater} || {scaled_main_heater}%."
    )
    pzr_cl1_spray = holding_reg.registers[1]
    scaled_cl1_spray = pzr_cl1_spray / 32767 * 100
    print(
        f"[CONTROL COMMAND] Reading control from PLC : UINT {pzr_cl1_spray} || {scaled_cl1_spray}%."
    )
    pzr_cl2_spray = holding_reg.registers[2]
    scaled_cl2_spray = pzr_cl2_spray / 32767 * 100
    print(
        f"[CONTROL COMMAND] Reading control from PLC : UINT {pzr_cl2_spray} || {scaled_cl2_spray}%."
    )
    ...
    print(
        f"[CONTROL COMMAND] Writing value to Asherah : UINT {pzr_main_heater} : Main Heater"
    )
    w_prz_cl1_spray_resp = mbans_client.write_register(4, pzr_cl1_spray, unit=1)
    print(
        f"[CONTROL COMMAND] Writing value to Asherah : UINT {pzr_cl1_spray} : Spray Valve 1."
    )
    w_prz_cl2_spray_resp = mbans_client.write_register(5, pzr_cl2_spray, unit=1)
    print(
        f"[CONTROL COMMAND] Writing value to Asherah : UINT {pzr_cl2_spray} : Spray Valve 2"
    )
    w_prz_backup_heater_resp = mbans_client.write_coil(3, pzr_backup_heater)
    print(
        f"[CONTROL COMMAND] Writing value to Asherah : BOOL {pzr_backup_heater} : Backup Heater"
    )
...
def connect_to_servers():
    print(
        f"[INFO] Attempting to connect to ANS on tcp://{mbans_srv_ip}:{mbans_srv_port}..."
    )
    global mbans_client
    global plc_client
...
@click.option(
    ...
    "mbans_ip",
    default="127.0.0.1",
    help="IP address of the simulation modbus server",
    show_default=True,
)
@click.option(
    "-p",
...
def main(mbans_ip, plc_ip):
    global mbans_srv_ip
    global plc_srv_ip
    mbans_srv_ip = mbans_ip
    plc_srv_ip = plc_ip
